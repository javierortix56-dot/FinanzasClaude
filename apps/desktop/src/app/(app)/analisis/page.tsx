...
import { getCatFromSettings, monthLabel, shiftMonth, getCurrentMonth } from '@finanzas/core/lib/constants'
...
export default function AnalisisPage() {
  ...
  useEffect(() => {
    // Flag de cancelación: al cambiar rápido de mes, un fetch viejo que
    // resuelve tarde no debe pisar los datos del mes nuevo.
    let active = true
    fetchMonthTransactions(shiftMonth(currentMonth, -1)).then((txs) => { if (active) setPrevTxs(txs) })
    fetchLastNMonths(6, currentMonth).then((h) => { if (active) setHistorico(h) })
    return () => { active = false }
  }, [currentMonth])
                    ...
                    {slices.length === 0 ? (
                      ...
                      <div key={s.id} className="flex items-center justify-between text-sm">
                        ...
                        <div className="text-right tabular-nums">
                          ...
                          <div className="text-xs text-muted">{totalCat > 0 ? ((s.value / totalCat) * 100).toFixed(0) : '0'}%</div>
                        </div>
                      </div>
                    ))}
                  </div>
...
function PilotoView({ ingresos, egresos, base, currentMonth }: { ingresos: number; egresos: number; base: Currency; currentMonth: string }) {
  ...
  // La proyección por ritmo de gasto solo tiene sentido para el mes en curso;
  // para meses pasados/futuros se muestran los valores reales.
  const isCurrent = currentMonth === getCurrentMonth()
  const today = new Date()
  const dom = today.getDate()
  const totalDays = new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate()
  const elapsed = dom / totalDays
  const proyIngresos = isCurrent && elapsed > 0 ? ingresos / elapsed : ingresos
  const proyEgresos  = isCurrent && elapsed > 0 ? egresos / elapsed : egresos
  const proyBalance  = proyIngresos - proyEgresos

  return (
    <div className="space-y-4">
      {!isCurrent && (
        <p className="text-xs text-muted">Las proyecciones solo están disponibles para el mes en curso; se muestran los valores reales.</p>
      )}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card>
          <CardContent className="pt-5">
            <div className="text-xs font-medium text-muted">{isCurrent ? 'Proyección Ingresos' : 'Ingresos del mes'}</div>
            ...
            <div className="text-xs text-muted mt-1">
              ...
              {isCurrent && <> · {(elapsed * 100).toFixed(0)}% del mes</>}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-5">
            <div className="text-xs font-medium text-muted">{isCurrent ? 'Proyección Egresos' : 'Egresos del mes'}</div>
        ...
        <Card>
          <CardContent className="pt-5">
            <div className="text-xs font-medium text-muted">{isCurrent ? 'Balance proyectado' : 'Balance del mes'}</div>
